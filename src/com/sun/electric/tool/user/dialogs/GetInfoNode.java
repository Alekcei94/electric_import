    ...
    private void doLoadInfo() {
        ...
        if (wnd != null) {
            for (Highlight h : wnd.getHighlighter().getHighlights()) {
                if (!h.isValid() || !h.isHighlightEOBJ()) {
                    continue;
                }
        ...
        swapXY = false;
        if (initialRotation == 900 || initialRotation == 2700) {
            swapXY = true;
        }
        ...
        if (ni.isCellInstance()) {
            editParameters.setEnabled(true);
        }
        ...
        if (attributes.isSelected() && !hasAttributes) {
            ports.setSelected(true);
        }
...
//			if (pnp.findMulticut() != null) multiCutNode = true;
            ...
            if (pnp.getFunction() == PrimitiveNode.Function.TRANMOSCN
                ...
                for (NodeLayer nodeLayer : primLayers) {
                    if (nodeLayer.getLayer().isCarbonNanotubeLayer()) {
                        extraFields.carbonNanotubeNode = true;
                    }
                }
            }
        }
    ...
    private static class ChangeNode extends Job {
        ...
        public boolean doIt() throws JobException {
            ...
            if (extrafields.extraField1 != null) {
                if (!currentTextField1.equals(initialTextField1)) {
                    if (currentTextField1.length() == 0) {
                        ni.delVar(extrafields.extraField1.getKey());
                    } else {
            ...
            if (extrafields.extraField2 != null) {
                if (!currentTextField2.equals(initialTextField2)) {
                    if (currentTextField2.length() == 0) {
                        ni.delVar(extrafields.extraField2.getKey());
                    } else {
            ...
            if (extrafields.carbonNanotubeNode) {
                if (!currentTextField1.equals(initialTextField1)) {
                    if (currentTextField1.equals("DEFAULT") || currentTextField1.length() == 0) {
                        ni.delVar(NodeLayer.CARBON_NANOTUBE_COUNT);
                    } else {
                ...
                if (!currentTextField2.equals(initialTextField2)) {
                    if (currentTextField2.equals("DEFAULT") || currentTextField2.length() == 0) {
                        ni.delVar(NodeLayer.CARBON_NANOTUBE_PITCH);
                    } else {
	...
	{//GEN-HEADEREND:event_seeActionPerformed
            ...
            if (wnd != null) {
                ...
                highlighter.clear();
                if (pi != null) {
                    highlighter.addElectricObject(pi, ni.getParent());
                } else {
