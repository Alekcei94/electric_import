...
/**
 * This is the "quick" DRC which does full hierarchical examination of the
 * circuit.
 * <P>
 * The "quick" DRC works as follows: It first examines every primitive node and
 * arc in the cell For each layer on these objects, it examines everything
 * surrounding it, even in subcells R-trees are used to limit search. Where cell
 * instances are found, the contents are examined recursively It next examines
 * every cell instance in the cell All other instances within the surrounding
 * area are considered When another instance is found, the two instances are
 * examined for interaction A cache is kept of instance pairs in specified
 * configurations to speed-up arrays All objects in the other instance that are
 * inside the bounds of the first are considered The other instance is
 * hierarchically examined to locate primitives in the area of consideration For
 * each layer on each primitive object found in the other instance, Examine the
 * contents of the first instance for interactions about that layer
 * <P>
 * Since Electric understands connectivity, it uses this information to
 * determine whether two layers are connected or not. However, if such global
 * connectivity is propagated in the standard Electric way (placing numbers on
 * exports, descending into the cell, and pulling the numbers onto local
 * networks) then it is not possible to decompose the DRC for multiple
 * processors, since two different processors may want to write global network
 * information on the same local networks at once.
 * <P>
 * To solve this problem, the "quick" DRC determines how many instances of each
 * cell exist. For every network in every cell, an array is built that is as
 * large as the number of instances of that cell. This array contains the global
 * network number for that each instance of the cell. The algorithm for building
 * these arrays is quick (1 second for a million-transistor chip) and the memory
 * requirement is not excessive (8 megabytes for a million-transistor chip). It
 * uses the CheckInst and CheckProto objects.
 *
...
public class Quick {
    ...
    /**
     * a NodeInst that is too tiny for its connection.
     */
    private NodeInst tinyNodeInst;
    /**
     * the other Geometric in "tiny" errors.
     */
    private Geometric tinyGeometric;
    /**
     * for tracking the time of good DRC.
     */
    private HashSet<Cell> goodSpacingDRCDate = new HashSet<Cell>();
    /**
     * for tracking cells that need to clean good DRC vars
     */
    private HashSet<Cell> cleanSpacingDRCDate = new HashSet<Cell>();
    /**
     * for tracking the time of good DRC.
     */
    private HashSet<Cell> goodAreaDRCDate = new HashSet<Cell>();
    /**
     * for tracking cells that need to clean good DRC vars
     */
    private HashSet<Cell> cleanAreaDRCDate = new HashSet<Cell>();
    /**
     * Top cell for DRC
     */
    private Cell topCell;
    /**
     * Miscellaneous data for DRC
     */
...
//    /* for figuring out which layers are valid for DRC */
     ...
     *
     * Method to do a hierarchical DRC check on cell "cell". If "count" is zero,
     * check the entire cell. If "count" is nonzero, only check that many
     * instances (in "nodesToCheck") and set the entry in "validity" TRUE if it
     * is DRC clean.
     *
     * @param bounds if null, check entire cell. If not null, only check area in
     * bounds.
     * @param drcJob
     * @param onlyArea
     */
    ...
    private void doCheck(ErrorLogger errorLog, Cell cell, Geometric[] geomsToCheck, boolean[] validity,
        ...
        topCell = cell;
        /* Especially important for minArea checking */
...
//		if (totalErrors != 0 && totalErrors != -2) goodDRCDate.clear();
     ...
     */
    /**
     * Method to check the contents of cell "cell" with global network index
     * "globalIndex". Returns positive if errors are found, zero if no errors
     * are found, negative on internal error.
     *
     * @param cell
     * @param globalIndex
     * @param bounds
     * @return positive number if errors are found, zero if no errors are found
     * and check the cell, -1 if job was aborted and -2 if cell is OK with DRC
     * date stored.
     */
    private int checkThisCell(Cell cell, int globalIndex, Rectangle2D bounds) {
        ...
        for (Iterator<NodeInst> it = cell.getNodes(); it.hasNext();) {
            ...
            if (cp.cellChecked && !cp.cellParameterized) {
                continue;
            }
...
//            totalMsgFound = checkMinArea(cell);
        ...
        for (Iterator<NodeInst> it = cell.getNodes(); it.hasNext();) {
            ...
            if (ret) {
                ...
                if (reportInfo.errorTypeSearch == DRC.DRCCheckMode.ERROR_CHECK_CELL) {
                    break;
                }
            }
        }
        ...
        for (Iterator<ArcInst> it = cell.getArcs(); it.hasNext();) {
            ...
            if (bounds != null) {
                if (!ai.getBounds().intersects(bounds)) {
                    continue;
                }
            }
...
//            System.out.println("DRC Exclusion found");
    ...
    /**
     * Private method to check if geometry given by Poly object is inside an
     * exclusion region
     *
     * @param poly
     * @param parent
...
//			ret = !isTransistor && checkSelectOverPolysilicon(ni, layer, poly, cell);
    ...
    /**
     * Method to check the design rules about arcinst "ai". Returns true if
     * errors were found.
     */
...
//			ret = checkSelectOverPolysilicon(ai, layer, poly, ai.getParent());
    ...
    /**
     * Method to check the design rules about cell instance "ni". Only check
     * other instances, and only check the parts of each that are within
     * striking range. Returns true if an error was found.
     */
...
//        instanceInteractionMap.clear(); // part3. Doesn't help
        for (Iterator<Geometric> it = ni.getParent().searchIterator(searchBounds); it.hasNext();) {
            ...
            if (geom == ni) {
                continue; // covered by checkInteraction?
            }
...
//                System.out.println("No worst spacing distance found in Quick:checkCellInst");
    ...
    /**
     * Method to recursively examine the area "bounds" in cell "cell" with
     * global index "globalIndex". The objects that are found are transformed by
     * "uptrans" to be in the space of a top-level cell. They are then compared
     * with objects in "oNi" (which is in that top-level cell), which has global
...
//				Poly [] arcPolyList = tech.getShapeOfArc(ai);
    ...
    /**
     * Method to examine polygon "poly" layer "layer" network "net" technology
     * "tech" geometry "geom" which is in cell "cell" and has global index
     * "globalIndex". The polygon is compared against things inside node "oNi",
     * and that node's parent has global index "topGlobalIndex".
     */
...
//        boolean baseMulti = tech.isMultiCutInTechnology(multiCutData);
    ...
    /**
     * Method to examine a polygon to see if it has any errors with its
     * surrounding area. The polygon is "poly" on layer "layer" on network "net"
     * from technology "tech" from object "geom". Checking looks in cell "cell"
     * global index "globalIndex". Object "geom" can be transformed to the space
     * of this cell with "trans". Returns TRUE if a spacing error is found
     * relative to anything surrounding it at or below this hierarchical level.
     */
    ...
    /**
     * Method to recursively examine a polygon to see if it has any errors with
     * its surrounding area. The polygon is "poly" on layer "layer" from
     * technology "tech" on network "net" from object "geom" which is associated
     * with global index "globalIndex". Checking looks in the area
     * (lxbound-hxbound, lybound-hybound) in cell "cell" global index
     * "cellGlobalIndex". The polygon coordinates are in the space of cell
     * "topCell", global index "topGlobalIndex", and objects in "cell" can be
     * transformed by "upTrans" to get to this space. The base object, in "geom"
     * can be transformed by "trans" to get to this space. The minimum size of
     * this polygon is "minSize" and "baseMulti" is TRUE if it comes from a
     * multicut contact. If the two objects are in the same cell instance
     * (nonhierarchical DRC), then "sameInstance" is TRUE. If they are from
     * different instances, then "sameInstance" is FALSE.
     *
        ...
        for (Iterator<Geometric> it = cell.searchIterator(bounds); it.hasNext();) {
            ...
            if (nGeom instanceof NodeInst) {
                ...
                if (ni.isCellInstance()) {
                    ...
                    if (np.getTechnology() != tech) {
                        continue;
                    }

                    // see if this type of node can interact with this layer
                    if (!validLayers.checkLayerWithNode(layer, np)) {
                        continue;
                    }
                ...
                if (!validLayers.checkLayerWithArc(layer, ap)) {
                    continue;
                }
...
//				Poly [] subPolyList = tech.getShapeOfArc(ai);
    ...
    /**
     * Function to detect small edges due to overlapping polygons of the same
     * material. It is valid only for manhattan shapes
     *
    ...
    /**
     * Method to compare: polygon "poly1" layer "layer1" network "net1" object
     * "geom1" with: polygon "poly2" layer "layer2" network "net2" object
     * "geom2" The polygons are both in technology "tech" and are in the space
     * of cell "cell" which has global index "globalIndex". Note that to
     * transform object "geom1" to this space, use "trans1" and to transform
     * object "geom2" to this space, use "trans2". They are connected if "con"
     * is nonzero. They cannot be less than "dist" apart (if "edge" is nonzero,
     * check edges only) and the rule for this is "rule".
     *
...
//            if (pd1 != pd)
        ...
        if (theRule.ruleType != DRCTemplate.DRCRuleType.SURROUND) {
            ...
            /**
             * Special case: ignore spacing errors generated by VT layers if
             * poly is already part of a VTH-transistors. Not very elegant
             * solution but should work for now.
             */
...
//                    }
     ...
     */
    /**
     * Method to examine, in cell "cell", the "count" instances in
     * "geomsToCheck". If they are DRC clean, set the associated entry in
     * "validity" to TRUE.
     */
...
//        double worstInteractionDistance = reportInfo.worstInteractionDistance;
    ...
    /**
     * Method to check primitive object "geom" (an arcinst or primitive
     * nodeinst) against cell instance "ni". Returns TRUE if there are
     * design-rule violations in their interaction.
     */
...
//            nodeInstPolyList = DRC.getShapeOfArcBasedOnRules(tech, oAi);
        ...
        for (int j = 0; j < tot; j++) {
            ...
            if (badBoxInArea(poly, polyLayer, tech, net, geom, trans, globalIndex, subBounds, (Cell) np, localIndex,
                    subCell, globalIndex, subTrans, multiCutData, false)) {
                return true;
            }
        }
        return false;
...
//		}
    ...
    /**
     * Method to recursively examine the hierarchy below cell "cell" and fill in
     * the CheckInst objects on every cell instance. Uses the CheckProto objects
     ...
     */
    /**
     * Function to look for conditional layers that will 100% overlap with the
     * given region.
     *
     * @param poly
     * @param layer
    ...
    /**
     * Method to ensure that polygon "poly" on layer "layer" from object "geom"
     * in technology "tech" meets minimum width rules. If it is too narrow,
     * other layers in the vicinity are checked to be sure it is indeed an
     * error. Returns true if an error is found.
     */
    private boolean checkMinWidth(Geometric geom, Layer layer, Poly poly, boolean onlyOne) {
        ...
        if (minWidthRule != null) {
            ...
            if (!errorDefault) {
                return false; // the default condition is the valid one.
            }
        }
...
//	}
    private void traversePolyTree(Layer layer, PolyBase.PolyBaseTree obj, int level, DRCTemplate minAreaRule,
        ...
        if (checkMin) {
            ...
            if (!DBMath.isGreaterThan(minVal, area)) {
                return; // larger than the min value
            }
            count.increment();
    ...
    private int checkMinAreaLayerWithTree(LayoutMerger merge, Cell cell, Layer layer) {
        ...
        if (minAreaRule == null && encloseAreaRule == null && spacingRule == null) {
            return 0;
        }
    ...
    /**
     * Method to ensure that polygon "poly" on layer "layer" from object "geom"
     * in technology "tech" meets minimum area rules. Returns true if an error
     * is found.
     */
...
//    }
    /**
     * Method to find two points between polygons "poly1" and "poly2" that can
     * be used to test for notches. The points are returned in (xf1,yf1) and
     * (xf2,yf2). Returns zero if no points exist in the gap between the
     * polygons (becuase they don't have common facing edges). Returns 1 if only
     * one of the reported points needs to be filled (because the polygons meet
...
//		pt2.setLocation((selectedPts1[1].getX() + selectedPts2[1].getX()) / 2, (selectedPts1[1].getY() + selectedPts2[1].getY()) / 2);
     ...
     * Point must be contained in the line x = (b2-b1)*a/(a^2 -1)
     *
...
//				}
    ...
    /**
     * Method to calculate the edge whose bounding box is the closest to a given
     * point.
     */
    ...
    /**
     * Method to calculate the closest point to the center of a given bounding
     * box
     */
     ...
     * geometry on layer "layer" (in or below cell "cell"). Returns true if
     * there is. If "needBoth" is true, both points must have geometry,
     * otherwise only 1 needs it.
     */
    ...
    /**
     * Method to examine cell "cell" in the area (lx<=X<=hx, ly<=Y<=hy) for
     * objects on layer "layer". Apply transformation "moreTrans" to the
     * objects. If polygons are found at (xf1,yf1) or (xf2,yf2) or (xf3,yf3)
     * then sets "p1found/p2found/p3found" to 1. If all locations are found,
     * returns true.
     */
...
//                        assert(false); // should this happen?
                    ...
                    for (j = 0; j < length; j++) {
                        if (pointsFound[j]) {
                            continue; // point covered
                        }
                        if (poly.isInside(pts[j])) {
                            pointsFound[j] = true;
                        }
                    }
...
//                DRCTemplate.DRCRuleType.CUTSIZE);
    ...
    private boolean checkExtensionGateRule(Geometric geom, Layer layer, Poly poly, Layer nLayer,
        ...
        if (dp.ignoreExtensionRuleChecking) {
            return false;
        }
...
//    }
    /**
     * Method to search for a transistor connected to this network that is not
     * geom. This is to implement special rule in 90nm technology. MOVE TO
     * TECHNOLOGY class!! to be cleaner
     *
     * @param network
     * @param geom
     * @return v
...
//	}
     ...
     * @param geom
     * @param cell
     * @param extensionArea contains information about poly area outside of
     * nLayer
     * @param overlapArea
     * @param polyBnd
     */
...
//        boolean[] baseFound = new boolean[4];  // all false at the beginning
        ...
        for (DRCTemplate rule : rules) {
            if (rule.nodeName != null) {
                continue; // ignore these ones, only use to resize primitives
            }
            if (rule.condition == null) {
                continue; // must be conditional
            }
            if (!rule.name1.equals(layer.getName())) {
                continue; // it has to be the first name
            }
     ...
     */
    /**
     * Method to check if non-transistor polysilicons are completed covered by
     * N++/P++ regions
     */
...
//    }
    /**
     * Method to check if certain poly rectangle is fully covered by any select
     * regoin
     */
...
//    }
    ...
    /**
     * This method determines if one of the polysilicon polygons is covered by a
     * vth layer. If yes, VT{H/L}_{P/N}.S.2 doesn't apply
     *
     * @param polys
     * @param layers
     * @param geoms
    ...
    /**
     * Method to look for network in m1 that would allow consider the poly
     * layers as maytouch
     *
     * @return
     */
    ...
    private boolean activeOnTransistor(Poly poly1, Layer layer1, int net1,
        ...
        for (int i = 0; i < 2; i++) {
            int j = (i + 1) % 2;
            if (!funs[i].isDiff()) {
                if (!funs[i].isContact() || (extras[i] & Layer.Function.CONDIFF) == 0) {
                    foundFalse = true; // return false;
                }
            } else {
        ...
        if (foundFalse) {
            return false; // need to make sure possible true value are not reach before returning false.
        }
...
//                continue;
    ...
    /**
     * Method to crop the box on layer "nLayer", electrical index "nNet" and
     * bounds (lx-hx, ly-hy) against the nodeinst "ni". The geometry in nodeinst
     * "ni" that is being checked runs from (nlx-nhx, nly-nhy). Only those
     * layers in the nodeinst that are the same layer and the same electrical
     * network are checked. Returns true if the bounds are reduced to nothing.
     */
...
//                assert(false); // should this happen?
    ...
    /**
     * Method to crop away any part of layer "lay" of arcinst "ai" that
     * coincides with a similar layer on a connecting nodeinst. The bounds of
     * the arcinst are in the reference parameters (lx-hx, ly-hy). Returns false
...
//		if (ni.getNumExports() == 0) return;
    ...
    /**
     * Method to determine the minimum distance between "layer1" and "layer2" in
     * technology "tech" and library "lib". If "con" is true, the layers are
     * connected. Also forces connectivity for same-implant layers. Returns
     * conditional rules if not standard rules were found
     */
...
//    }
    /**
     * Method to return the network number for port "pp" on node "ni", given
     * that the node is in a cell with global index "globalIndex".
     */
    ...
    /**
     * Method to recursively scan cell "cell" (transformed with "trans")
     * searching for DRC Exclusion nodes. Each node is added to the global list
     * "exclusionList".
     */
    ...
    private abstract class QuickAreaEnumerator extends HierarchyEnumerator.Visitor {
        ...
        /**
         * Method to check whether the layer must be checked or not. It might be
         * done by this layer on that network
         *
         * @param layer
    ...
    private class QuickAreaEnumeratorSlow extends QuickAreaEnumerator {
        ...
        public boolean visitNodeInst(Nodable no, HierarchyEnumerator.CellInfo info) {
            ...
            if (isTransistor) {
                ...
                if (!pNp.getFunction().isCNTransistor() && !pNp.getFunction().isBipolar()) {
                    assert (list.length == 1);
                }
                activePoly = list[0];
            }
